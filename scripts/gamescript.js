...
let CanvasHeightOffset = window.innerHeight * 0.2;
canvas.height = window.innerHeight - CanvasHeightOffset;
...
*/
let len = (canvas.width + (canvas.height + CanvasHeightOffset)) * 0.04;
if (canvas.height < canvas.width) {
    len = ((canvas.height + CanvasHeightOffset)) * 0.1;
} else if (canvas.width < canvas.height) {
    len = (canvas.width) * 0.1;
}

let xpos = canvas.width / 2 - len * 2;
let ypos = (canvas.height + CanvasHeightOffset) / 2.8 - len * 3;
let boxColor = "transparent";
const playerColor = "ghostwhite";
const winLineColor = "#b82601";
const fontColor = "#f3f3f3";
// const borderColor = "#222222";
const borderColor = "#f3f3f3";

tic = new TicBoard(xpos, ypos, len);
button = new Button(tic.posx + tic.len * 1.2, tic.posy + tic.len * 5, tic.len * 1.5);
...
window.addEventListener("resize", function () {
    CanvasHeightOffset = window.innerHeight * 0.2;
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight - CanvasHeightOffset;

    tic.len = (canvas.width + (canvas.height + CanvasHeightOffset)) * 0.04;
    if (canvas.height < canvas.width) {
        tic.len = ((canvas.height + CanvasHeightOffset)) * 0.1;
    } else if (canvas.width < canvas.height) {
        tic.len = (canvas.width) * 0.1;
    }

    tic.posx = canvas.width / 2 - tic.len * 2;
    tic.posy = (canvas.height + CanvasHeightOffset) / 2.8 - tic.len * 3;
...
function clearBoard(reInit) {
    ...
    document.getElementById("tictactoeContent").children[0].innerHTML = "";
...
function gameResult() {
    let crossResult = hasWon(cross);
    let circleResult = hasWon(circle);

    if (crossResult[0] == -1 || circleResult[0] == -1) {
        document.getElementById("tictactoeContent").children[0].innerHTML = "No Winners!";
        offsetY = canvas.getBoundingClientRect().top;
        offsetX = canvas.getBoundingClientRect().left;
    } else if (crossResult[0]) {
        document.getElementById("tictactoeContent").children[0].innerHTML = "Cross Wins!";
        drawWin(crossResult[1]);
        crossTurn = true;
        offsetY = canvas.getBoundingClientRect().top;
        offsetX = canvas.getBoundingClientRect().left;
    } else if (circleResult[0]) {
        document.getElementById("tictactoeContent").children[0].innerHTML = "Circle Wins!";
